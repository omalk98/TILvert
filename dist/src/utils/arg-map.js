...
function processArguments(argv) {
    ...
    let input = ".";
    let lastIndex = 0;
    exports.CLIArgumentMap.forEach((arg) => {
        ...
        if (longArgIndex !== -1 || shortArgIndex !== -1) {
            ...
            if (longArgIndex !== -1 &&
                ...
                if (longArgIndex >= lastIndex)
                    lastIndex = longArgIndex + 1;
            }
            else if (shortArgIndex !== -1 &&
                shortArgIndex + 1 < argv.length &&
                !arg.is_flag &&
                argv[shortArgIndex + 1][0] !== "-") {
                arg_map.set(arg.key, argv[shortArgIndex + 1]);
                if (shortArgIndex >= lastIndex)
                    lastIndex = shortArgIndex + 1;
            }
            else {
            ...
            arg_map.set(arg.key, arg.default);
    });
    if (lastIndex + 1 < argv.length)
        input = argv[argv.length - 1];
